(function () {
    'use strict';
    candidateApp.controller('NavigationController', NavigationController);
    NavigationController.$inject = ['$rootScope', '$scope', '$location', 'NavigationService', 'ToastrService', '$state', '$mdPanel','$mdDialog', 'CandidateUsersService'];
    function NavigationController($rootScope, $scope, $location, NavigationService, ToastrService, $state, $mdPanel,$mdDialog, CandidateUsersService) {
        ...
        if($rootScope.isCandidate && $rootScope.CandidateInfo){
            //vm.getHireInfo()
            var allSaved = true;
            for(var i=0; i<$rootScope.CandidateInfo.newhireworkflowwebforms.length; i++){
                if($rootScope.CandidateInfo.newhireworkflowwebforms[i].subwebforms && $rootScope.CandidateInfo.newhireworkflowwebforms[i].subwebforms.length){
                    
                    for(var j=0; j<$rootScope.CandidateInfo.newhireworkflowwebforms[i].subwebforms.length; j++){
                        if(!$rootScope.CandidateInfo.newhireworkflowwebforms[i].subwebforms[j].isSaved){
                            allSaved = false;
                            break;
                        }
                    }
                } else {
                    if(!$rootScope.CandidateInfo.newhireworkflowwebforms[i].isSaved){
                        allSaved = false;
                        break;
                    }
                }
...
//($rootScope.UserInfo.isAdmin ? 'Settings' : 'Settings.Documents')
        ...
        vm.candidateMenuItems = [
            { Title: 'INTRODUCTION', State: 'introduction', Svg: 'images/icons/newhire_icon.svg' },
            { Title: 'DASHBOARD', State: 'dashboard', Svg: 'images/icons/dashboard_icon.svg' },
        ];
